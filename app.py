...
def get_recommendations():
    ...
    try:
        ...
        if (static_sim == "") and (dynamic_sim == ""):
            ...
            json_data = similarity_functions.present_json(
                query_id, quest_cmb, simulate=False
            )
            ...
            print(
                "Calling Dynamic similarity computations and building predictive visualizations!!"
            )
            ...
            # Both together
            quest_cmb = parameters["sim-sel-grp"]
            json_data_static = similarity_functions.present_json(
                query_id, quest_cmb, simulate=False
            )
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if not Path(properties.questions_location + "user_id.json").is_file():
                ...
                with open(
                    "".join(
                        properties.data_location + "user_questions/user_id" + ".json"
                    ),
                    "w",
                ) as f:
                    json.dump(id, f)
        ...
        try:
            ...
            print(
                "Saving user questionnaire to the location ",
                properties.questions_location,
            )
            with open(
                "".join(
                    properties.questions_location
                    + str(user_id_json["user_id"])
                    + ".json"
                ),
                "w",
            ) as f:
            ...
            with open(
                "".join(properties.questions_location + "user_id.json"), "w"
            ) as f:
                json.dump(user_id_json, f)
            return render_template("success.html")
        except Exception:
            print(traceback.print_exc())
            return render_template("error.html", error=traceback.print_exc())
...
def switch_plot():
    ...
    if plot_type == "boxplot":
        ...
        graph_data = similarity_functions.create_boxplot_compare(
            query_list, var_type, simulate=False
        )
...
def get_information():
    ...
    try:
        ...
        if plot_type == "heatmap":
            data_output = similarity_functions.get_patient_information(
                combination, query_id, nearest_pid, simulate=False
            )
            ...
            output = {}
            data_hm_output = similarity_functions.get_patient_information(
                combination, query_id, nearest_pid, simulate=False
            )
...
def predict():
    ...
    for method in ["mean", "min", "max"]:
        if method == "mean":
            ...
            predict_json = similarity_functions.compute_linear_regression(
                user_id, stress, nearest_n, float(time_point), user_tsg_mean
            )

        elif method == "min":
            # Min of nearest neighbors
            predict_json = similarity_functions.compute_linear_regression(
                user_id, stress, nearest_n, float(time_point), user_tsg_min
            )
        else:
            # Max of nearest neighbors
            predict_json = similarity_functions.compute_linear_regression(
                user_id, stress, nearest_n, float(time_point), user_tsg_max
            )
...
def explore_dash():
    ...
    nobs_users = plot_methods.create_number_of_observations_user(
        user_ids[0], usr_years[0], months_sel[0], "bar"
    )
    ...
    graph_monthly_overview = plot_methods.plot_monthly_overview(
        user_overview_summary, user_ids[0]
    )
...
def get_details_nobs():
    ...
    user_results = time_data_set_f[
        (time_data_set_f["user_id"] == int(user_id))
...
def change_k_plot():
    ...
    try:
        ...
        if static_sim and (not dynamic_sim):
            ...
            print("Calling static similarity and building predictive visualization!!")
            json_data = similarity_functions.present_json(
                user_id, quest_cmb, k=k_val, simulate=False
            )
